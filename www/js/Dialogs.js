...
function AboutDialog(editorUi)
{
	...
	mxUtils.br(div);
	mxUtils.write(div, 'Modified By XiaoZhang ');
	mxUtils.br(div);
	mxUtils.write(div, 'QQ: 546499741');
	mxUtils.br(div);
	mxUtils.write(div, 'Email: s_jqzhang@163.com');
	mxUtils.br(div);
...
function LoginDialog(editorUi)
{
	...
	div.setAttribute('align', 'center');
	var h3 = document.createElement('h3');
	mxUtils.write(h3,"Login");
	div.appendChild(h3);


	var loginForm='<table> <tr> <td>username:</td> <td><input  id="username" type="text"></td> </tr> <tr> <td>password:</td> <td><input id="pwd" type="password"></td> </tr> <tr> <td colspan="2"><input id="btnCancel" value="Cancel" type="button"><input value="Login" id="btnLogin" type="button"></td> </tr> </table>';
	...
	$('#btnCancel',$(div)).click(function(){
		editorUi.hideDialog();
	});
	$('#btnLogin',$(div)).click(function(){
		login();
	});

	$('#pwd',$(div)).keyup(function(key){
	if(key.	keyCode==13){
		login();
	}
});
...
function ExportDialog(editorUi)
{
	...
	var nameInput = document.createElement('input');
	nameInput.setAttribute('value', $('#graph_name').html());
	nameInput.style.width = '180px';
	...
	td.appendChild(mxUtils.button(mxResources.get('save'), mxUtils.bind(this, function()
	{
		...
		else
		{
	        ...
	        else
	        {
				...
				if (true||xml.length <= MAX_REQUEST_SIZE && width < MAX_WIDTH && width > 0 &&
